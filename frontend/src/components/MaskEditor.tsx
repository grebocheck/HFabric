...
export function MaskEditor({
  ...
  return (
    <div className="rounded-md border border-white/10 bg-black/20 p-2.5">
      ...
      <div className="mt-2 flex justify-center overflow-hidden rounded-md border border-white/10 bg-black/30">
        <div className="relative grid max-h-52 max-w-full touch-none select-none">
          <img
            src={src}
            alt="source"
            draggable={false}
            onLoad={(e) => setSize({ x: e.currentTarget.naturalWidth || 1, y: e.currentTarget.naturalHeight || 1 })}
            className="col-start-1 row-start-1 max-h-52 max-w-full object-contain"
          />
          <canvas
            ref={canvasRef}
            width={size.x}
            ...
            className="col-start-1 row-start-1 max-h-52 max-w-full cursor-crosshair opacity-55 mix-blend-screen"
          />
          {lassoPoints.length > 1 ? (
            <svg
              viewBox={`0 0 ${size.x} ${size.y}`}
              className="pointer-events-none col-start-1 row-start-1 max-h-52 max-w-full"
            >
              <polyline
        ...
        {(["brush", "lasso", "erase"] as Tool[]).map((item) => (
          <button
            ...
            className={`${toolButton} ${
              tool === item ? "border-accent/70 bg-accent/20 text-white" : "border-white/15 text-white/60 hover:bg-white/10"
            }`}
          >
            {item}
          </button>
        ))}
        <button type="button" onClick={undo} disabled={!canUndo} className={`${toolButton} border-white/15 text-white/60 hover:bg-white/10`}>
          Undo
        </button>
        <button type="button" onClick={clear} disabled={!hasMask} className={`${toolButton} border-white/15 text-white/60 hover:bg-white/10`}>
          Clear
        </button>
      </div>
      <div className="mt-2 grid gap-2">
        <div>
          <div className="flex items-center justify-between text-[11px] text-white/40">
            <span>Brush</span>
            <span className="font-mono text-white/60">{Math.round(brush)}px</span>
          </div>
        ...
        <div>
          <div className="flex items-center justify-between text-[11px] text-white/40">
            <span>Feather</span>
            <span className="font-mono text-white/60">{Math.round(feather)}px</span>
          </div>
