...
function ThinkingPending({ active }: { active: boolean }) {
  return (
    <div
      role="status"
      aria-live="polite"
      className="ui-card flex items-center gap-3 rounded-md px-3 py-2"
    >
      <span className="relative h-8 w-8 shrink-0">
        <span className="absolute inset-0 rounded-full border border-accent/20" />
        <span className="absolute inset-1 rounded-full border-2 border-border border-t-accent animate-spin" />
        <span className="absolute inset-[11px] rounded-full bg-accent/80 shadow-[0_0_14px_var(--color-accent-glow)] animate-pulse" />
      </span>
      <span className="min-w-0">
        <span className="block text-xs font-medium text-ui-muted">{active ? "Thinking" : "Preparing reply"}</span>
...
function Thinking({ reasoning, active }: { reasoning: string; active: boolean }) {
  ...
  return (
    <div className="ui-card mb-2 rounded-md">
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex w-full items-center gap-2 px-2.5 py-1.5 text-left text-xs text-ui-subtle transition hover:bg-control-hover hover:text-ui"
      >
        {active ? (
      ...
      {expanded ? (
        <div className="whitespace-pre-wrap border-t border-border px-2.5 py-2 text-xs leading-5 text-ui-muted">
          {reasoning}
        </div>
      ) : null}
    </div>
