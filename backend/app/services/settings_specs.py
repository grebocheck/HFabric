"""Settings override schema metadata."""
...
SPECS: tuple[SettingSpec, ...] = (
    ...
    SettingSpec(
        "llama_port", "Chat port", "llm", "integer", minimum=1, maximum=65535, step=1, restart_required=True
    ),
    SettingSpec(
        "llama_embed_port",
        "Embedding port",
    ...
    SettingSpec(
        "llama_ngl", "GPU layers", "llm", "integer", "999 means full offload.", minimum=0, maximum=999, step=1
    ),
    ...
    SettingSpec("qwen_image_img2img_strength", "Qwen edit strength", "image_defaults", "number", minimum=0.05, maximum=1, step=0.01),
    SettingSpec("z_image_img2img_strength", "Z-Image edit strength", "image_defaults", "number", minimum=0.05, maximum=1, step=0.01),
    SettingSpec("anima_img2img_strength", "Anima edit strength", "image_defaults", "number", minimum=0.05, maximum=1, step=0.01),
    SettingSpec("img2img_min_effective_steps", "Minimum edit steps", "image_defaults", "integer", minimum=1, maximum=20, step=1),
    SettingSpec("image_edit_resize_mode", "Edit resize mode", "image_defaults", "choice", choices=_choices(("crop", "pad", "stretch"))),
    SettingSpec("inpaint_padding_mask_crop", "Inpaint crop padding", "image_defaults", "integer", minimum=0, maximum=512, step=8),
    SettingSpec("inpaint_mask_blur", "Mask blur", "image_defaults", "number", minimum=0, maximum=128, step=1),
    SettingSpec("inpaint_mask_grow", "Mask grow", "image_defaults", "integer", minimum=-128, maximum=128, step=1),
    SettingSpec(
        "image_upload_max_mb", "Image upload MB", "image_defaults", "integer", minimum=1, maximum=2048, step=1
    ),
    SettingSpec(
    ...
    SettingSpec("sdxl_controlnet_depth_repo", "SDXL ControlNet depth", "image_defaults", "text", nullable=True, restart_required=True),
    SettingSpec("sdxl_controlnet_pose_repo", "SDXL ControlNet pose", "image_defaults", "text", nullable=True, restart_required=True),
    SettingSpec("sdxl_controlnet_scribble_repo", "SDXL ControlNet scribble", "image_defaults", "text", nullable=True, restart_required=True),
    SettingSpec("sdxl_controlnet_union_repo", "SDXL ControlNet Union", "image_defaults", "text", nullable=True, restart_required=True),
    # Family defaults
    SettingSpec(
        "anima_default_steps", "Anima steps", "family_defaults", "integer", minimum=1, maximum=150, step=1
    ),
    SettingSpec(
        "anima_default_guidance",
        "Anima guidance",
        "family_defaults",
        "number",
        minimum=0,
    ...
    SettingSpec(
        ...
        "family_defaults",
        "choice",
        choices=_choices(("bnb-nf4", "bnb-fp4", "none")),
    ),
    SettingSpec(
        "flux2_offload",
        "FLUX.2 offload",
        "family_defaults",
        "choice",
        choices=_choices(("model", "sequential", "none")),
    ),
    SettingSpec(
        "flux2_default_steps", "FLUX.2 steps", "family_defaults", "integer", minimum=1, maximum=150, step=1
    ),
    SettingSpec(
        "flux2_default_guidance",
        "FLUX.2 guidance",
        "family_defaults",
        "number",
        minimum=0,
    ...
    SettingSpec(
        "qwen_image_default_steps", "Qwen steps", "family_defaults", "integer", minimum=1, maximum=150, step=1
    ),
    SettingSpec(
        "qwen_image_default_guidance",
        "Qwen guidance",
        "family_defaults",
        "number",
        minimum=0,
    ...
    SettingSpec(
        "qwen_image_edit_default_guidance",
        "Qwen Edit guidance",
        "family_defaults",
        "number",
        minimum=0,
    ...
    SettingSpec(
        "flux_kontext_offload",
        "FLUX Kontext offload",
        "family_defaults",
        "choice",
        choices=_choices(("model", "sequential", "none")),
    ),
    SettingSpec(
        "flux_kontext_default_steps",
        "FLUX Kontext steps",
        "family_defaults",
        "integer",
        minimum=1,
    ...
    SettingSpec(
        "flux_kontext_default_guidance",
        "FLUX Kontext guidance",
        "family_defaults",
        "number",
        minimum=0,
        maximum=30,
    ...
    SettingSpec(
        "z_image_default_guidance",
        "Z-Image Turbo guidance",
        "family_defaults",
        "number",
        minimum=0,
        maximum=30,
    ...
    SettingSpec(
        "z_image_base_default_guidance",
        "Z-Image base guidance",
        "family_defaults",
        "number",
        minimum=0,
        maximum=30,
    ...
    SettingSpec(
        ...
        "acceleration",
        "choice",
        choices=_choices(("default", "reduce-overhead", "max-autotune")),
    ),
    ...
    SettingSpec(
        ...
        "acceleration",
        "choice",
        choices=_choices(("off", "fb", "teacache")),
    ),
    SettingSpec(
    ...
    SettingSpec(
        ...
        "acceleration",
        "choice",
        choices=_choices(("auto", "flash", "efficient", "math", "cudnn")),
    ),
    ...
    SettingSpec(
        ...
        "acceleration",
        "choice",
        choices=_choices(("highest", "high", "medium")),
    ),
    SettingSpec(
        "sdxl_turbo_lora",
        "SDXL turbo LoRA",
    ...
    SettingSpec(
        "sdxl_turbo_steps", "SDXL turbo steps", "acceleration", "integer", minimum=1, maximum=50, step=1
    ),
    SettingSpec(
        "sdxl_turbo_guidance",
        "SDXL turbo guidance",
    ...
    SettingSpec(
        "image_lora_cache_max", "LoRA cache max", "acceleration", "integer", minimum=0, maximum=32, step=1
    ),
    SettingSpec(
    ...
    SettingSpec(
        "keep_warm_max_models", "Warm model limit", "memory", "integer", minimum=0, maximum=8, step=1
    ),
    SettingSpec(
    ...
    SettingSpec(
        "min_free_ram_gb", "Minimum free RAM GB", "memory", "number", minimum=0.5, maximum=128, step=0.5
    ),
    SettingSpec(
        "mem_poll_seconds", "Memory poll seconds", "memory", "number", minimum=0.5, maximum=60, step=0.5
    ),
    SettingSpec("learn_memory_profiles", "Learn memory profiles", "memory", "boolean"),
    SettingSpec(
        "learned_ram_margin_gb", "Learned RAM margin GB", "memory", "number", minimum=0, maximum=64, step=0.1
    ),
    # Speech, RAG, attachments
    SettingSpec("tts_gpu_layers", "TTS GPU layers", "tools", "integer", minimum=0, maximum=999, step=1),
    SettingSpec(
        "tts_timeout_seconds", "TTS timeout seconds", "tools", "integer", minimum=30, maximum=7200, step=30
    ),
    SettingSpec(
        ...
        "tools",
        "choice",
        choices=_choices(("cpu", "cuda", "auto")),
    ),
    SettingSpec(
        ...
        "tools",
        "choice",
        choices=_choices(("int8", "int8_float16", "float16", "float32")),
    ),
    SettingSpec(
    ...
    SettingSpec(
        "embed_gpu_layers", "Embedding GPU layers", "tools", "integer", minimum=0, maximum=999, step=1
    ),
    SettingSpec(
    ...
    SettingSpec(
        "rag_chunk_overlap", "RAG chunk overlap", "tools", "integer", minimum=0, maximum=2000, step=10
    ),
    ...
    SettingSpec(
        "voice_max_upload_mb", "Voice upload MB", "voice", "integer", minimum=1, maximum=1024, step=1
    ),
    ...
    SettingSpec(
        ...
        "voice",
        "choice",
        choices=_choices(("fcpe", "rmvpe", "crepe_tiny", "crepe_full")),
    ),
    SettingSpec(
        "voice_input_highpass_hz", "Input high-pass Hz", "voice", "integer", minimum=0, maximum=300, step=5
    ),
    ...
    SettingSpec(
        "voice_input_denoise_mix", "Input denoise mix", "voice", "number", minimum=0, maximum=1, step=0.01
    ),
    # Advanced sources
    SettingSpec("anima_support_dir", "Anima support dir", "sources", "path", restart_required=True),
    SettingSpec("anima_text_encoder_path", "Anima text encoder", "sources", "path", restart_required=True),
    SettingSpec("anima_qwen_config_dir", "Anima Qwen config", "sources", "path", restart_required=True),
    SettingSpec("anima_t5_tokenizer_dir", "Anima T5 tokenizer", "sources", "path", restart_required=True),
    SettingSpec("anima_vae_dir", "Anima VAE dir", "sources", "path", restart_required=True),
    ...
    SettingSpec(
        "z_image_nunchaku_offload",
        "Z-Image nunchaku offload",
        "sources",
        "choice",
        choices=_choices(("model", "none")),
    ),
